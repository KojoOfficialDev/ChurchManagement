...
type AddCommunionFormProps = {
  form: UseFormReturn<Communion>
}

export const AddCommunionForm = ({ form }: AddCommunionFormProps) => {
  const {
  ...
  const section = useMemo(() => {
    const baseSection = COMMUNION_FORM_SECTIONS[0]
  ...
  const shouldShowField = useCallback(
    (field: FormField) => {
      ...
      const dependsOnValue = form.watch(field.dependsOn as FieldPath<Communion>)
  ...
  const shouldShowChildSection = useCallback(
    (childSection: FormSection) => {
      ...
      const dependsOnValue = form.watch(
        childSection.dependsOn as FieldPath<Communion>,
      )
  ...
  const handleSave = async (data: Communion) => {
    await mutateAsync(data, {
      onSuccess: () => {
        form.reset()
      },
      onError: () => {
        toast.error('Failed to create communion record')
      },
    })
  }
