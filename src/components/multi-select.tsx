...
interface MultiSelectProps
  ...
  /**
   * If true, allows users to create new options when search value doesn't match existing options.
   * Optional, defaults to false.
   */
  allowCreate?: boolean

  /**
   * Optional callback function called when creating a new option.
   * Receives the search value and should return a MultiSelectOption or Promise<MultiSelectOption>.
   * If not provided, a default option with { label: searchValue, value: searchValue } will be created.
   */
  onCreate?: (
    searchValue: string,
  ) => MultiSelectOption | Promise<MultiSelectOption>
}
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
  (
    ...
    const getAllOptions = React.useCallback((): Array<MultiSelectOption> => {
      ...
      // Merge created options with provided options
    ...
    }, [options, createdOptions, deduplicateOptions, isGroupedOptions])
    ...
    const filteredOptions = React.useMemo(() => {
      ...
      // If no search or not searchable, return all options as flat list
      if (!searchable || !searchValue) {
        // Return as flat list (created options are not grouped)
        return allOptions
      }

      if (allOptions.length === 0) return []

      // Filter all options based on search
    ...
    }, [searchValue, searchable, getAllOptions])

    const searchValueExists = React.useCallback((): boolean => {
      if (!searchValue) return true
      const allOptions = getAllOptions()
      return allOptions.some(
        (option) =>
          option.label.toLowerCase() === searchValue.toLowerCase() ||
          String(option.value).toLowerCase() === searchValue.toLowerCase(),
      )
    }, [searchValue, getAllOptions])

    const handleCreateOption = React.useCallback(async () => {
      if (!allowCreate || !searchValue || searchValueExists()) return
      ...
      if (onCreate) {
        ...
        try {
          newOption = await onCreate(searchValue)
        } catch (error) {
      ...
      announce(
        `New option "${newOption.label}" created and selected. ${newSelectedValues.length} options selected.`,
        'assertive',
      )
    }, [
      allowCreate,
    ...
    const handleInputKeyDown = (
      ...
      if (event.key === 'Enter') {
        // If allowCreate is enabled and search value doesn't exist, create it
        if (allowCreate && searchValue && !searchValueExists()) {
        ...
        >
          ...
          >
            <Command>
              ...
              >
                <CommandEmpty className="py-2" asChild>
                  {allowCreate && searchValue && !searchValueExists() ? (
                    <button
                      onClick={handleCreateOption}
                      className="cursor-pointer justify-center flex items-center gap-2 px-4 w-full italic h-full"
                    >
