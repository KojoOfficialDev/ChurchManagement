...
import {
	...
	GetSocietiesList,
	SearchMemberList,
} from '../../../core/services/member.services';
...
export const ExpenseCategoryList = () => {
	useRedirectToAdminPage('expensecategory/expensecategorylist');
	...
	const DeleteRecord = async (id: number) => {
		...
		ExpenseCategoryListQuery?.refetch();
	};
	const ExpenseCategoryListQuery = useQuery({
		retry: (count) => count < 1,
		//enabled:false,
		staleTime: Infinity,
		queryKey: ['expenseTransactionsQuery'],
		queryFn: () => GetExpensesCategoryList(isExportRequest).then((res) => res.data),
		onSuccess: (data) => {
			setRows(data);
	...
	useEffect(() => {
		dispatch(setAppLoading(ExpenseCategoryListQuery?.isFetching));
	}, [ExpenseCategoryListQuery.isFetching]);
	...
	const handleExportRows = (rowsToExport: MRT_Row<any>[]) => {
		...
		doc.save('Expenses Type List ' + moment().format('LLL'));
		...
		<>
			<div className='relative p-4'>
				<div className='flex flex-row justify-between items-center w-full'>
					<p className='font-medium text-2xl pb-4'>Expenses Type List</p>
					<div>
						<MtnButton
							label='Add Expenses Type'
