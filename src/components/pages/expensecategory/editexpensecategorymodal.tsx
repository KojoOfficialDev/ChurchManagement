...
const EditExpenseCategoryModal: React.FC<EditExpenseCategoryModalProps> = ({
	isOpen,
	onClose,
	rowData,
	loading,
	...
	const onFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
		...
		if (ExpenseCategory.name == '') {
			toast.Error('Error', 'Please Enter Expense Category Name');
			return;
		}
		...
		try {
			const response = await fetch('https://catholicportal.net/api/ExpensesCategory/Update', {
				method: 'POST',
				headers: {
			...
			if (response.ok) {
				...
				swal('Great! Expense Type has been updated!', {
					icon: 'success',
				});
				...
				toast.Error('Error', 'Failed to update Expense Type. Please try again.');
			}
		} catch (error) {
										...
										<form onSubmit={onFormSubmit}>
											<div className='flex flex-row gap-6 w-full items-center'>
												...
												<div className='mb-4 w-1/7'>
													<Checkbox
														label='Active'
														checked={ExpenseCategory.active}
														onChange={(e) =>
															setExpenseCategory({
																...ExpenseCategory,
