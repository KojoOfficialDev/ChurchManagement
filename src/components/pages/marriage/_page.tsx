...
export const MarriageList = () => {
	...
	const formatMarriageData = (data: Marriage[]) => {
		return data.map(marriage => ({
			...
			nuptialBlessingDate: marriage.nuptialBlessingDate ? moment(marriage.nuptialBlessingDate).format(dateFormat) : 'N/A',
		}));
	};
	...
	const handleExportRows = (rowsToExport: Marriage[]) => {
		const header = {
			marriageNumber: "Marriage Number",
			coupleName: "Couple Name",
			marriageDate: "Marriage Date",
	...
	return (<>
		...
		<AddMarriageModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} loading={isLoadingModalOpen} done={() => setIsLoadingModalOpen(false)} />
		<EditMarriageModal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} rowData={selectedRowData} loading={isLoadingModalOpen} done={() => setIsLoadingModalOpen(false)} />
	</>
