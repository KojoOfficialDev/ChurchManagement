...
const EditMemberModal: React.FC<EditMemberModalProps> = ({
	...
	const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			formData.dateOfBirth = new Date(selectedDate).toISOString();
	...
	const capture = () => {
		const file = dataURLtoFile(webcamRef.current.getScreenshot(), `${formData.membershipNumber}.jpg`);
		setSelectedImage(file);
		formData.imageUrl = webcamRef.current.getScreenshot() as string;
	};
												...
												<div>
													...
													<div className='flex flex-row gap-10 w-full'>
														<div className='mb-4 w-1/2'>
															...
															<DatePicker
																value={new Date(formData.dateOfBirth).toISOString()}
																	...
																	{societies.map((option) => (
																		...
																		>
																			<input
																				...
																				className='rounded w-4 h-4 py-2 px-3'
																				checked={formData.societyName.includes(
																					option.id,
																				)}
																				disabled={isView}
																			/>
