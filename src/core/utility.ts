...
export const getUserSession = () => {
    try {
        let user = localStorage.get('u_user_info');
...
export function validator(rules: { rule: validatorRules; message: string; data?: any }[], value: any, formCollection?: HTMLFormControlsCollection) {
    for (const rule of rules) {
        ...
        if (ruleKey === 'compare' && (formCollection && value != (formCollection[data] as HTMLFormElement)?.value)) return message;
...
export const isFormValid = (formId: string, validations: IBulkValidation, callback?: (type: string, data: any) => void): boolean => {
